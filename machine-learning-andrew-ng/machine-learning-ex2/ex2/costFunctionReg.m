...
%J = -1/m*(y'*log(sigmoid(X(:,2:size(X,2))*theta(2:size(theta))))+(1-y)'*log((1-sigmoid(X(:,2:size(X,2))*theta(2:size(theta)))))) + lambda/(2*m)*((theta(2:size(theta)))'*theta(2:size(theta)));
